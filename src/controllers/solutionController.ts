...
export const findSolution = async (
  ...
  try {
    const rsolutionDto = plainToClass(SolutionFindDto, req.body);
    const errors = await validate(rsolutionDto, {
    ...
    const result = await solutionService.findSolution(req.body);

    return res.json(result);
  } catch (e) {
    console.error('Error find solution', e);
    return res.status(500).send((e as Error).message);
  }
}